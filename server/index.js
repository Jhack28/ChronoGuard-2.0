...
const pool = mysql.createPool({
    ...
    app.post('/admin', async (req, res) => {
        try {
            ...
            const [result] = await pool.execute(
                'INSERT INTO Usuarios (Nombre, Correo, Contraseña, ID_Rol, Numero_de_Documento, id_departamento, Estado) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [nombre, email, hashedPassword, rol, numero_de_documento, id_departamento, 'Activo']
            );
    ...
    app.put('/usuarios/inactivar/:id', async (req, res) => {
        try {
            const { id } = req.params;
            await pool.execute(
